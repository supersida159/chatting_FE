...
const Inputpage: FC<inputProps> = ({ className, children, variant, isLoading, size, ...props }) => {
  const [current, setCurrent] = useState<string | null>('');

  // Define and export two handleOnChange functions
  ...
  return (
    <input className={cn(InputVariant({ variant, className }))} disabled={isLoading} onChange={handleOnChange} {...props}>
      {children}
    </input>
  );
};
